...
for i = 1:iter
    ...
    if alpha > 0 && alpha < 1
        deno(1,:,:,:) = max(alpha,sqrt(grad_next(1,:,:,:).^2 + grad_next(2,:,:,:).^2 + grad_next(3,:,:,:).^2));
        deno(2,:,:,:) = deno(1,:,:,:);
        deno(3,:,:,:) = deno(1,:,:,:);
        deno(4,:,:,:) = max(1-alpha,sqrt(grad_next(4,:,:,:).^2 + grad_next(5,:,:,:).^2 + grad_next(6,:,:,:).^2));
        deno(5,:,:,:) = deno(4,:,:,:);
        deno(6,:,:,:) = deno(4,:,:,:);
    ...
    elseif alpha == 0
        deno(4,:,:,:) = max(1,sqrt(grad_next(4,:,:,:).^2 + grad_next(5,:,:,:).^2 + grad_next(6,:,:,:).^2));
        deno(5,:,:,:) = deno(4,:,:,:);
        deno(6,:,:,:) = deno(4,:,:,:);
