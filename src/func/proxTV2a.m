...
for i = 1:iter
    ...
    if alpha > 0 && alpha < 1
        deno(1,:,:,:,1) = max(alpha,abs(grad_next(1,:,:,:,1)));
        deno(1,:,:,:,2) = max(alpha,abs(grad_next(1,:,:,:,2)));
        deno(1,:,:,:,3) = max(alpha,abs(grad_next(1,:,:,:,3)));
        deno(2,:,:,:,1) = max(1-alpha,abs(grad_next(2,:,:,:,1)));
        deno(2,:,:,:,2) = max(1-alpha,abs(grad_next(2,:,:,:,2)));
        deno(2,:,:,:,3) = max(1-alpha,abs(grad_next(2,:,:,:,3)));
        grad_next = grad_next./deno;
        grad_next(1,:,:,:,:) = grad_next(1,:,:,:,:)*alpha;
        grad_next(2,:,:,:,:) = grad_next(2,:,:,:,:)*(1-alpha);
    elseif alpha == 0
        deno(2,:,:,:,1) = max(1,abs(grad_next(2,:,:,:,1)));
        deno(2,:,:,:,2) = max(1,abs(grad_next(2,:,:,:,2)));
        deno(2,:,:,:,3) = max(1,abs(grad_next(2,:,:,:,3)));
        grad_next(1,:,:,:,:) = zeros(n1,n2,n3,3);
        grad_next(2,:,:,:,:) = grad_next(2,:,:,:,:)./deno(2,:,:,:,:);
    elseif alpha == 1
        deno(1,:,:,:,1) = max(1,abs(grad_next(1,:,:,:,1)));
        deno(1,:,:,:,2) = max(1,abs(grad_next(1,:,:,:,2)));
        deno(1,:,:,:,3) = max(1,abs(grad_next(1,:,:,:,3)));
