...
for i = 1:iter
    ...
    deno(1,:,:,:,1) = max(1,sqrt(grad_next(1,:,:,:,1).^2 + grad_next(2,:,:,:,1).^2));
    deno(1,:,:,:,2) = max(1,sqrt(grad_next(1,:,:,:,2).^2 + grad_next(2,:,:,:,2).^2));
    deno(1,:,:,:,3) = max(1,sqrt(grad_next(1,:,:,:,3).^2 + grad_next(2,:,:,:,3).^2));
    deno(2,:,:,:,1) = deno(1,:,:,:,1);
    deno(2,:,:,:,2) = deno(1,:,:,:,2);
    deno(2,:,:,:,3) = deno(1,:,:,:,3);
